...
class DatePicker {
    constructor() {
        ...
        this.options = {
            ...
            nextArrow: `<svg viewBox="0 0 6 10" xmlns="http://www.w3.org/2000/svg">
        <path d="M1.69209 0.273249C2.05245 0.624986 5.57328 4.32082 5.57328 4.32082C5.76553 4.50962 5.86208 4.75532 5.86208 5.00102C5.86208 5.24672 5.76553 5.49242 5.57328 5.6795C5.57328 5.6795 2.05245 9.37706 1.69209 9.72707C1.33173 10.0788 0.683431 10.1029 0.299795 9.72707C-0.0847026 9.35292 -0.114876 8.82962 0.299795 8.37012L3.53009 5.00102L0.299795 1.63192C-0.114876 1.17242 -0.0847026 0.648263 0.299795 0.273249C0.683431 -0.102628 1.33173 -0.0793515 1.69209 0.273249Z"/>
            ...
            prevArrow: `<svg viewBox="0 0 6 10" xmlns="http://www.w3.org/2000/svg">
    ...
    onInit() {
        const datePicker = flatpickr('.js-datepicker', this.options);
    }
}
...
class CalendarInline {
    ...
    onInit() {
        const datePicker = flatpickr(
            this.calendar.querySelector('.js-calendar-inline'),
            this.calendarOptions
        );
    }
    ...
    getYear(selectedDates, dateStr, instance) {
        const elementYear = this.calendar.querySelector('.calendar-inline__year');

        elementYear.textContent = instance.currentYear;
    }

    renderWidget(date, month, day) {
        const dayElement = this.widget.querySelector('.calendar-widget__dateday');
        const monthElement = this.widget.querySelector('.calendar-widget__month');
        const weekElement = this.widget.querySelector('.calendar-widget__weekday');

        dayElement.textContent = date;
        monthElement.textContent = calendarMonth[month];
        weekElement.textContent = this.weekDay[day];
    }

    createEvents(dObj, dStr, fp, day) {
...
class CalendarFull {
    ...
    closeAllEvent(event) {
        if (!event.target.closest('.c-event')
            && !event.target.closest('.dropdown-c-event')) {
            this.events.forEach(el => {
    ...
    reminderRemove(event) {
        ...
        $(target).slideUp(200, () => target.remove());
    }
...
const echartsElements = [];

class ChartProgressCircle {
    constructor() {
        this.element = document.querySelectorAll('.js-progress-circle');

        if (this.element[0]) {
            this.element.forEach(this.onInit.bind(this));
        }
    }
...
class Echart {
    ...
    init(options) {
        ...
        echartsElements.push(chartInstance);
    }

    static resize() {
        echartsElements.forEach(el => el.resize());
    }

    static tooltip(formatter) {
        return {
            trigger: 'axis',
            axisPointer: {
                type: 'shadow'
    ...
    constructor() {
        this.revenueLineChartOptions = {
            series: [{
                ...
                data: ChartInstance.dataSeries('#revenueChart')[0]
            }, {
                name: 'Page Views',
                data: ChartInstance.dataSeries('#revenueChart')[1]
            }],
            chart: {
                height: '100%',
                type: 'area',
        ...
        this.profitPieChartOptions = {
            series: ChartInstance.dataSeries('#profitPieChart'),
            ...
            xAxis: [
                {
                    ...
                    data: ChartInstance.dataSeries('#activityChart')[0],
                    axisLabel: {
                        fontSize: 12,
                        fontFamily: themeStyle('--font-family-default'),
                        color: themeStyle('--text-secondary-color')
            ...
            series: [
                {
                    ...
                    type: 'bar',
                    barGap: 0,
                    data: ChartInstance.dataSeries('#activityChart')[1],
                    itemStyle: {
                ...
                },
                {
                    name: 'Comment',
                    type: 'bar',
                    data: ChartInstance.dataSeries('#activityChart')[2],
                    itemStyle: {
                        borderRadius: [5, 5, 0, 0]
                    }
                }
        ...
        this.taskRarialChartOptions = {
            series: ChartInstance.dataSeries('#taskPieChart'),
            chart: {
                type: 'radialBar',
                toolbar: {
                    show: false
        ...
        this.overviewLineChartOptions = {
            series: [{
                name: 'Session Duration',
                data: ChartInstance.dataSeries('#overviewLineChart')
            }],
            chart: {
                height: '100%',
                type: 'area',
        ...
        this.statisticsBarChartOptions = {
            grid: {
                show: false,
                left: '54',
                right: '0',
            ...
            xAxis: [
                {
                    ...
                    data: ChartInstance.dataSeries('#statisticsBarChart')[0],
                    axisLabel: {
                        fontSize: 12,
                        fontFamily: themeStyle('--font-family-default'),
                        color: themeStyle('--text-secondary-color')
            ...
            series: [
                {
                    ...
                    type: 'bar',
                    barGap: 0,
                    data: ChartInstance.dataSeries('#statisticsBarChart')[1],
                    itemStyle: {
                        borderRadius: [5, 5, 0, 0]
                    }
                },
                {
                    name: 'Target',
                    type: 'bar',
                    data: ChartInstance.dataSeries('#statisticsBarChart')[2],
                    itemStyle: {
                        borderRadius: [5, 5, 0, 0]
                    }
                }
        ...
        new Charts('revenueChart', this.revenueLineChartOptions);
        new Charts('profitPieChart', this.profitPieChartOptions);
        new Echart('activityChart', this.activityBarEChartOptions);
        new Charts('taskPieChart', this.taskRarialChartOptions);
        new Charts('overviewLineChart', this.overviewLineChartOptions);
        new Echart('statisticsBarChart', this.statisticsBarChartOptions);
    }

    static dataSeries(el) {
...
class CreditCard {
    constructor() {
        this.card = document.querySelectorAll('.credit-card');
        this.creditCardNumber = fitty('.credit-card__number-item');
        this.creditCardDate = fitty('.credit-card__date span', {
            minSize: 11
        });

        this.card.forEach(this.onInit.bind(this));
    }

    onInit(card) {
        card.querySelectorAll('.credit-card__number-item')[0].addEventListener('fit', event => {
            const cardNumber = card.querySelector('.credit-card__number-wrapper');

            cardNumber.style.fontSize = `${Math.floor(event.detail.newValue)}px`;
        });

        card.querySelectorAll('.credit-card__date span')[0].addEventListener('fit', event => {
            const cardName = card.querySelector('.credit-card__name');
            const cardDate = card.querySelector('.credit-card__date');

            cardName.style.fontSize = `${Math.floor(event.detail.newValue)}px`;
            cardDate.style.fontSize = `${Math.floor(event.detail.newValue)}px`;
        });

        [...card.querySelectorAll('.credit-card__number-item')].forEach((el) => {
            el.addEventListener('fit', event => {
                const element = event.target;
...
class HeaderSearch {
    ...
    close(event) {
        if (this.toggleButton.classList.contains('active')
            && !event.target.closest('.header__search')
            && !event.target.closest('.header__toggle-search')) {
...
class ModuleSearch {
    ...
    onInit() {
        this.toggleButton.forEach((el, index) => {
            el.addEventListener('click', event => this.toggle(event, index));
        });
        document.addEventListener('click', event => this.close(event));
    }
    ...
    close(event) {
        if (this.getActiveElement !== null
            && this.search.classList.contains('show')) {
            if (this.toggleButton[this.getActiveElement].classList.contains('active')
                && !event.target.closest('.module-search')
                && !event.target.closest('.toggle-search--module')) {
...
class MapInstance {
    ...
    onInit() {
        const instance = $(this.element).vectorMap({
            map: 'world_en',
...
class AddProduct {
    ...
    onInit() {
        const productSlider = this.element;
        ...
        const mainSlider = new Swiper(this.mainSliderElement, {
            ...
            on: {
                slideChangeTransitionStart: swiper => swiper.update()
            }
        });

        productSlider.addEventListener('product-gallery.close', () => {
            mainSlider.slideTo(+productSlider.dataset.activeSlide, 0);
        });
    }
}
...
class ProductCart {
    constructor() {
        ...
        this.products = document.querySelectorAll('.product-cart');
        this.productsLenght = 0;

        this.products.forEach(this.onInit.bind(this));
    }

    onInit(el, index, arr) {
        const product = el;
        const counterElement = product.querySelector('.product-cart__quantity input');
        const priceElement = product.querySelector('.product-cart__price');
        const buttonRemove = product.querySelector('.product-cart__remove');
        const priceSymbol = priceElement.textContent.slice(0, 1);
        const price = priceElement.textContent.slice(1) / counterElement.value;

        this.productsLenght = arr.length;
        counterElement.addEventListener('input', event => ProductCart.counter(event, priceElement, priceSymbol, price));
        buttonRemove.addEventListener('click', event => this.removeItem(event, product));
    }
    ...
    removeItem(event, target) {
        ...
        $(target).fadeOut(300, () => {
            ...
            if (this.productsLenght === 0) {
                const cart = document.querySelector('.cart');
                const cartEmptyTitle = document.querySelector('.cart-empty__title');
                const cartEmptyButton = document.querySelector('.cart-empty__button');
                const tl = gsap.timeline();

                cart.classList.add('is-empty');

                gsap.set(cartEmptyTitle, {
                    clipPath: 'polygon(0 0, 0 0, 0 100%, 0 100%)'
                });

                gsap.set(cartEmptyButton, {
                    autoAlpha: 0,
                    y: 20
                });

                tl.to(cartEmptyTitle, {
    ...
    static counter(event, priceElement, priceSymbol, priceNumber) {
        ...
        price.textContent = `${priceSymbol}${input.value * priceNumber}`;
    }
}
...
class Checkboxes {
    ...
    onInit() {
        ...
        this.element.forEach((el) => {
            ...
            const inputs = document.querySelectorAll(`[data-checkbox="${checkboxAll.dataset.checkboxAll}"]`);
    ...
    checkedAll(event) {
        ...
        const inputs = document.querySelectorAll(`[data-checkbox="${checkbox.dataset.checkboxAll}"]`);
        const inputsChecked = [];
    ...
    checkedInput(event) {
        const input = event.currentTarget;
        const checkboxAll = document.querySelector(`[data-checkbox-all="${input.dataset.checkbox}"]`);
        const inputs = document.querySelectorAll(`[data-checkbox="${checkboxAll.dataset.checkboxAll}"]`);
        const inputsChecked = [];
...
class Timeline {
    constructor() {
        ...
        this.items = document.querySelectorAll('.timeline-item__wrapper');
    }
        ...
        if (this.elements[0]) {
            ...
            this.items.forEach(el => {
                ...
                gsap.to(element, {
                    scrollTrigger: {
                        trigger: element,
                        start: window.innerWidth < 992 ? '0 100%' : '20% 100%'
                    },
                    y: 0,
                    autoAlpha: 1,
                    duration: 0.5,
...
class Todo {
    constructor() {
        ...
        if (this.item[0]) {
            this.item.forEach(this.onInit.bind(this));
        }
    }
...
class SwitcherGroup {
    ...
    static getWidth(element, activeButton) {
        const elem = element;
...
class FormEditor {
    constructor(el) {
        this.formEditorIcons = Quill.import('ui/icons');
        this.elementId = el;
        this.options = {
            modules: {
                toolbar: [
                    [{ header: [1, 2, false] }],
                    [{ background: ['#0081FF', '#22CCE2', '#FD594A', '#09B66D', '#FF8A48'] }],
        ...
        this.formEditorIcons.background = `<svg viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
...
class Form {
    constructor() {
        ...
        this.addCreditCardForm = document.querySelector('.js-add-card-form');
    ...
    onInit() {
        ...
        if (this.addCreditCardForm) {
            this.addCreditCardForm.addEventListener('submit', event => onCreditAddCard(event));
        }

        document.querySelectorAll('.js-rating-stars').forEach(this.ratingInit.bind(this));
    ...
    ratingInit(el) {
        $(el).starRating({
            ...
            useFullStars: true,
            minRating: 1,
            readOnly: Boolean(el.dataset.readonly),
...
class Pagination {
    constructor() {
        ...
        if (this.pagination[0]) {
            this.pagination.forEach(this.onInit.bind(this));
        }
    }
    ...
    onInit(el) {
        ...
        links.forEach(linkEl => {
            linkEl.addEventListener('click', event => this.toggleItem(event));
        });
    }
...
class InputSelects {
    constructor(select) {
        ...
        if (this.inputs[0]) {
            this.inputs.forEach(this.onInit.bind(this));
        }
    }
    ...
    onInit(el) {
        ...
        $(element).select2({
            ...
            .on('select2:opening', event => InputSelects.destroySearch(event))
            .on('select2:open', event => InputSelects.scrollbar(event));
    }

    static scrollbar(event) {
    ...
    static destroySearch(event) {
        const element = event.currentTarget;

        element.parentNode.querySelectorAll('.select2-search, .select2-focusser').forEach(el => {
            el.remove();
        });
    }
}

class InputTags extends InputSelects {
    constructor(select) {
        super(select);
    }
...
class Tag {
    constructor() {
        ...
        if (this.labels) {
            this.labels.forEach(this.onInit.bind(this));
        }
    }
...
class Tooltips {
    constructor() {
        this.tooltipBoundary = document.querySelector('.page-content');
    ...
    onInit() {
        $('[data-toggle="tooltip"]').tooltip({
            ...
            container: this.tooltipBoundary,
