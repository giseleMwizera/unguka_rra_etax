...
document.addEventListener('DOMContentLoaded', function() {
    ...
    payerForm.addEventListener('submit', async function(event) {
        ...
        let {responseCode, responseMessage, responseObject } = await getDocDetails(event);
...
const formattedDate = `${year}-${month}-${day}`;
    ...
    userForm.addEventListener('submit', async function(event) {
        let {responseCode, responseMessage } = await submitForm(event);
        ...
        if(responseCode == 0) {
         ...
         title.textContent = responseMessage;
          title.style.color = "green";
            ...
            fields.forEach(function(fields) {
                const names = fields.querySelector('input[name="names[]"]').value;
                const account = fields.querySelector('input[name="account[]"]').value;
                const currency = fields.querySelector('#currency').value;
                console.log(currency);
    
                const email = fields.querySelector('input[name="email[]"]').value;
                const nid = fields.querySelector('input[name="NID[]"]').value;
                const tin = fields.querySelector('input[name="tin[]"]').value;
                const number = fields.querySelector('input[name="number[]"]').value;
    ...
    function createUserFields() {
        ...
        const inputFields = [
            { label: 'Names', type: 'text', name: 'names[]', placeholder: 'Type in the tax payer names', required: true },

            { label: 'Account', type: 'text', name: 'account[]', placeholder: 'Type in the tax payer account', required: true },

            { label: 'Tin', type: 'text', name: 'tin[]', placeholder: 'Type in the tax payer TIN', required: true },

            { label: 'Phone Number', type: 'number', name: 'number[]', placeholder: 'Type in the tax payer number', required: true },

            { label: 'Currency', type: 'select', name: 'currency[]', required: true,  id: 'currency', options: ['Select currency', 'USD', 'GBP', 'RWF'] },

            { label: 'National ID', type: 'number', name: 'NID[]', placeholder: 'Type in the tax payer ID', required: true },

            { label: 'Email', type: 'email', name: 'email[]', placeholder: 'Type in the tax payer email' },
        ];
